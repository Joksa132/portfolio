  ...
  return (
    <section className='flex flex-col items-center' id='about' ref={sectionRefs.about}>
      ...
      <div className='mt-5 flex flex-col items-center w-full'>
        <p className='text-lg w-4/6 max-md:text-sm max-[400px]:text-xs'>
        ...
        <div className="w-4/6 mt-10 grid gap-2">
          <div className={`first-row grid grid-cols-5 gap-2 opacity-0 max-sm:grid-cols-3 max-[450px]:grid-cols-2 ${isAboutVisible ? 'card-left-animation' : ''}`}>
            {firstFiveTechnologies.map((technology, index) => (
              <TechnologyCard key={index} techName={technology?.name} techImg={technology?.image} />
            ))}
          </div>
          <div className={`second-row grid grid-cols-5 gap-2 opacity-0 max-sm:grid-cols-3 max-[450px]:grid-cols-2 ${isAboutVisible ? 'card-right-animation' : ''}`}>
